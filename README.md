...
| 13 | TLS `key_share` will be X25519 only | caustic-crypto exports `p256_ecdh_base` but no `p256_ecdh_shared`, so P-256 key exchange needs work upstream. A server that insists on P-256 will get a named `TLS_ERR_HRR` rather than a mystery |
